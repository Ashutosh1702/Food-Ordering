  ...
  return (
    <SafeAreaView className="bg-white flex-1">
      <ScrollView
        ...
        contentContainerClassName="px-6 pt-7 pb-36 gap-7"
      >
        {/* Header card */}
        <View
        ...
        >
          <View className="flex-row items-center gap-4">
            ...
            <View className="flex-1">
              ...
              <Text className="paragraph-medium text-white/90">
                {user?.email ?? "—"}
              </Text>
            </View>
            <TouchableOpacity
        ...
        >
          <TouchableOpacity
            className="px-6 py-5"
            onPress={() =>
              Alert.alert("One Benefits", "Explore all One benefits")
            }
          >
            <View className="flex-row items-center gap-4">
              <View
              ...
              <View className="flex-1">
                <View className="flex-row items-center gap-2">
                  ...
                  >
                    <Text className="caption-medium text-white">ACTIVE</Text>
                  </View>
                ...
                <Text
                  className="caption-medium mt-1"
                  style={{ color: "#9CA3AF" }}
                >
                  Explore all benefits
                </Text>
              </View>
              <Text className="paragraph-medium" style={{ color: "#9CA3AF" }}>
                ›
              </Text>
            </View>
          </TouchableOpacity>
        ...
        <View className="flex-row justify-between">
          {[
            ...
            {
              title: "Payment Modes",
              onPress: () => router.push("/profile/payment-modes"),
            },
            {
              title: "My Refunds",
              onPress: () => Alert.alert("My Refunds", "Coming soon"),
            },
            {
              title: "Swiggy Money",
              onPress: () => Alert.alert("Swiggy Money", "Coming soon"),
            },
          ].map((item) => (
            <TouchableOpacity
              key={item.title}
            ...
            >
              ...
              <Text
                className="paragraph-medium text-dark-100 text-center"
                numberOfLines={1}
              >
                {item.title}
              </Text>
        ...
        <View className="border border-gray-200 rounded-2xl overflow-hidden">
          {[
            ...
            <TouchableOpacity
              key={row.title}
              className={`px-6 py-5 bg-white ${idx < 7 ? "border-b border-gray-200" : ""}`}
        ...
        <View className="gap-3">
          ...
          <TouchableOpacity
            className="self-start rounded-full px-4 py-2 bg-dark-100"
          ...
          <View className="flex-row bg-gray-100 rounded-full p-1 self-start">
            <TouchableOpacity className="px-4 py-2 rounded-full bg-white">
              <Text className="paragraph-medium text-dark-100">Food</Text>
            </TouchableOpacity>
            <TouchableOpacity className="px-4 py-2 rounded-full">
              <Text className="paragraph-medium text-gray-200">Instamart</Text>
            </TouchableOpacity>
          </View>
        </View>

        {/* Danger/Logout */}
        <View className="mt-6 border border-gray-200 rounded-2xl overflow-hidden bg-white">
          <TouchableOpacity className="px-6 py-5 bg-white" onPress={onLogout}>
